...
# opkg install getopt bc jq curl perl-image-exiftool rsync

# Check whether the required packages are installed
if [ ! -x "$(command -v getopt)" ] || [ ! -x "$(command -v bc)" ] || [ ! -x "$(command -v jq)" ] || [ ! -x "$(command -v curl)" ] || [ ! -x "$(command -v exiftool)"] || [ ! -x "$(command -v rsync)" ]; then
    echo "Make sure that the following tools are installed on your system: getopt, bc, jq, curl, exiftool, rsync"
    exit 1
fi
